...
#define KEY3 16756815 // both above
...
#define carSpeed 100 // changed from 250 to 150 to 100
...
void getBTData() {
  if(Serial.available()) {
    switch(Serial.read()) {
      ...
      case '3': func_mode = BothAbove;                      break;
      default:  break;
    } 
  }
}
...
void getIRData() {
  if (irrecv.decode(&results)){ 
    ...
    switch(results.value){
      ...
      case KEY3:  func_mode = BothAbove;                   break;
      default: break;
    }
    irrecv.resume();
  }
...
// edited this but it doesn't change anything
void line_teacking_mode() {
  if(func_mode == LineTeacking){
    if(LineTeacking_Read_Right == HIGH && (LineTeacking_Read_Left == LOW)) {
      right();
      while(LineTeacking_Read_Right == HIGH && (LineTeacking_Read_Left == LOW));
    }
    else if(LineTeacking_Read_Left == HIGH && (LineTeacking_Read_Right == LOW)) {
      left();
      while(LineTeacking_Read_Left == HIGH && (LineTeacking_Read_Right == LOW));
    }
    else if (LineTeacking_Read_Middle == HIGH){
      forward();
    }
  }  
}
...
// attempting to do obstacle avoidance with line tracking
void both_mode() {
  if(func_mode == BothAbove){
    middleDistance = getDistance();
